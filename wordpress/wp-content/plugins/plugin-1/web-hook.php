...
function register_webhook_trigger() {
    // Trigger webhook for new or updated posts and pages
    add_action('save_post', 'webhook_on_post_save', 10, 3);

    // Send all existing posts and pages to webhook when plugin is activated for the first time or reactivated
    if (get_option('webhook_plugin_activated') !== 'true') {
        send_all_posts_to_webhook();
        send_all_pages_to_webhook();
...
// Function to send all pages to webhook
function send_all_pages_to_webhook() {
    $args = array(
        'post_type' => 'page', // Change to your post type if necessary
        'post_status' => 'publish',
        'posts_per_page' => -1,
    );
    $pages = get_pages($args);
    foreach ($pages as $page) {
        send_page_to_webhook($page->ID);
    }
}
...
function send_post_to_webhook($post_id) {
    ...
    $payload = array();
    // Prepare payload for posts
    $payload = array(
        'post_id' => $post_id,
        'post_title' => $post->post_title,
        'post_content' => $post->post_content,
        'post_url' => get_permalink($post_id),
        // Add any other data you want to include in the payload for posts
    );
...
function send_page_to_webhook($post_id) {
    // Fetch post data
    $post = get_post($post_id);
...
function webhook_on_page_save($post_id, $post, $update) {
    // Check if this is not an autosave and the post is published or updated
    if (!$update || ($update && $post->post_status === 'publish')) {
        // Ignore revisions
        if (wp_is_post_revision($post_id)) {
